...
import py
from history import ResOperation, MergePoint, Jump, BoxInt, ConstInt
from codegen386.runner import CPU, GuardFailed
from pypy.rpython.lltypesystem import lltype
from codegen386.test.test_runner import FakeMetaInterp, FakeStats
...
def test_longevity_after_merge_point():
    cpu = CPU(rtyper=None, stats=FakeStats())
    meta_interp = FakeMetaInterp()
    cpu.set_meta_interp(meta_interp)
    i = BoxInt(0) # a loop variable
    ...
    flag_0 = BoxInt(1) # True
    operations = [
        MergePoint('merge_point', [i, flag, j], []),
        ResOperation('guard_false', [flag], []),
        ResOperation('int_lt', [i, ConstInt(5)], [flag_0]),
        Jump('jump', [ConstInt(3), flag_0, ConstInt(1)], []),
        ]
    startmp = operations[0]
    operations[-1].jump_target = startmp
    ...
    cpu.compile_operations(operations)
    res = cpu.execute_operations_in_new_frame('foo', startmp,
                                             [BoxInt(0), BoxInt(1), BoxInt(3)],
                                              'int')
    assert res.value == 42
    # assert stuff
    regalloc = cpu.assembler._regalloc
    # no leakage, args to merge_point
    assert regalloc.current_stack_depth == 3
    longevity = regalloc.longevity
    assert longevity[j] == (0, 1)
    assert meta_interp.recordedvalues == [0, True, 3]
