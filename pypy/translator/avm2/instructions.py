...
class _Avm2ShortInstruction(object):
    specialized=False
    def __init__(self, opcode, name, stack=0, scope=0, flags=0, **kwargs):
        self.opcode = opcode
        self.name   = name
        self.stack  = stack
        self.scope  = scope
    ...
    def specialize(self, **kwargs):
        return type("_Avm2_Instruction_%s" % self.name,
                    ...
                    dict(kwargs.items(),
                         specialized=True,
                         set_assembler_props=self.__class__._set_assembler_props.im_func,
                         serialize=self.__class__._serialize.im_func,
                         __init__=dummy,
                         __repr__=self.__class__._repr.im_func,
                         opcode=self.opcode,
                         name=self.name,
                         stack=self.stack,
                         scope=self.scope,
...
class _Avm2U30Instruction(_Avm2U8Instruction):
    ...
    def __call__(self, argument, *arguments):
        if arguments:
            return self.specialize(argument=([argument] + list(arguments)))
        return self.specialize(argument=argument)
...
class _Avm2MultinameInstruction(_Avm2U30Instruction):
    @needs_specialized
    def _set_assembler_props(self, asm):
        super(_Avm2MultinameInstruction, self)._set_assembler_props(asm)
        has_rtns      = asm.constants.has_RTNS(self.multiname)
        has_rtname    = asm.constants.has_RTName(self.multiname)
        if hasattr(self, "no_rt") and self.no_rt and (has_rtname or has_rtns):
            raise ValueError("getlex used with runtime-qualified name: %s" % (self.multiname))
...
class _Avm2SetLocalInstruction(_Avm2U30Instruction):
    def __call__(self, index):
        ...
        if index in _speed:
            return _speed[index].specialize()
        return self.specialize(argument=index)
    
class _Avm2GetLocalInstruction(_Avm2U30Instruction):
    def __call__(self, index):
...
class _Avm2OffsetInstruction(_Avm2ShortInstruction):
    ...
    @needs_specialized
    def _set_assembler_props(self, asm):
        super(_Avm2OffsetInstruction, self)._set_assembler_props(asm)
    ...
    def __call__(self, name):
        return self.specialize(lblname=name)
...
class _Avm2LabelInstruction(_Avm2ShortInstruction):
    ...
    def __call__(self, name):
        return self.specialize(lblname=name, define=False)
...
class _Avm2CallIDX(_Avm2U30Instruction):
    ...
    def _set_assembler_props(self, asm):
        ...
        self.argument = [self.index, self.num_args]
        asm.stack_depth += 1 - (self.num_args + 1) # push object/args; push result
        
    def __call__(self, multiname, num_args):
        return self.specialize(multiname=multiname, num_args=num_args)

class _Avm2CallMN(_Avm2CallIDX):
    is_void = False
    @needs_specialized
    def _set_assembler_props(self, asm):
        super(_Avm2CallMN, self)._set_assembler_props(asm)
        has_rtns   = asm.constants.has_RTNS(self.multiname)
        has_rtname = asm.constants.has_RTName(self.multiname)
...
jump = _Avm2OffsetInstruction(0x10, 'jump')
#}
...
coerce = _Avm2MultinameInstruction(0x80, 'coerce', 0, no_rt=True)
getlex = _Avm2MultinameInstruction(0x60, 'getlex', 1, no_rt=True)
deleteproperty = _Avm2MultinameInstruction(0x6A, 'deleteproperty', 0)
getdescendants = _Avm2MultinameInstruction(0x59, 'getdescendants', 0)
getproperty = _Avm2MultinameInstruction(0x66, 'getproperty', 0)
getsuper = _Avm2MultinameInstruction(0x04, 'getsuper', 0)
findproperty = _Avm2MultinameInstruction(0x5E, 'findproperty', 1)
findpropstrict = _Avm2MultinameInstruction(0x5D, 'findpropstrict', 1)
initproperty = _Avm2MultinameInstruction(0x68, 'initproperty', -2)
setproperty = _Avm2MultinameInstruction(0x61, 'setproperty', -2)
setsuper = _Avm2MultinameInstruction(0x05, 'setsuper', -2)
