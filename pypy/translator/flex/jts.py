...
from pypy.rpython.ootypesystem.bltregistry import BasicExternal, MethodDesc, ExternalType
...
class JTS(object):
    ...
    def obj_name(self, obj):
        #import pdb
        #pdb.set_trace()
        if isinstance(obj, ExternalType):
            basic_external = obj._class_
            if not hasattr(basic_external, "_render_class"):
                raise TypeError("This class %s can not be instantiated." % (basic_external,))
            return basic_external._render_class
