...
class BitStream(object):
    ...
    def read_fixed_value(self, length, eight_bit_mantissa=False):
        ...
        return self.read_int_value(length) / (float(0x100) if eight_bit_mantissa else float(0x10000))
    ...
    def write_fixed_value(self, value, length=-1, eight_bit_mantissa=False):
        ...
        self.write_int_value(int(value * (0x100 if eight_bit_mantissa else 0x10000)), length)
    ...
    def serialize(self, align=ALIGN_RIGHT, endianness="<"):
        ...
        if leftover > 0:
            if align == ALIGN_RIGHT:
                ...
                list += [False] * (8-leftover)
        list = BitStream(list)
        bytes = [list.read_int_value(8) for i in range(int(math.ceil(len(list)/8.0)))]
