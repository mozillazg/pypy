...
def frameloc_esp(offset, WORD=4):
    assert offset % WORD == 0
    if offset >= 0:
        return LOC_ESP_PLUS | offset
    else:
        # On 64-bit, the "red zone" might need access to e.g. -8(%esp).
        # But such positions should never be used across a CALL...
        raise AssertionError(offset)

def frameloc_ebp(offset, WORD=4):
    assert offset % WORD == 0
    if offset >= 0:
...
class LocalVarAbstract(object):
    ...
    def __cmp__(self, other):
        if isinstance(other, LocalVarAbstract):
    ...
    def getlocation(self, framesize, uses_frame_pointer):
        if (self.hint == 'esp' or not uses_frame_pointer
            ...
            if ofs_from_esp % 2 == 0:
                return frameloc_esp(ofs_from_esp, self.WORD)
            # we can get an odd value if the framesize is marked as bogus
            # by visit_andl()
        assert uses_frame_pointer
        ofs_from_ebp = self.ofs_from_frame_end + self.WORD
        return frameloc_ebp(ofs_from_ebp, self.WORD)

class LocalVar32(LocalVarAbstract):
    WORD = 4

class LocalVar64(LocalVarAbstract):
    WORD = 8
...
class InsnPrologue(Insn):
    def __setattr__(self, attr, value):
        if attr == 'framesize':
            assert value in (4,8), ("unrecognized function prologue - "
